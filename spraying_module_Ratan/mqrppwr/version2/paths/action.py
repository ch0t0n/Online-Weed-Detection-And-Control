...
def get_action(G, path, vl, omega):
	...
	for i in range(len(path)-1):
		...
		if G._node[current]['kind']==0:
		...
		elif G._node[current]['kind']==1:
			s_time = G._node[current]['cost']
			...
			move_time = edge_cost.dist_cost(current[1], next[1], vl)
			action.append(('M', move_time))
		elif  G._node[current]['kind']==2:
			f_time = G._node[current]['cost']
