...
def subtourelim(model, where):
	try:
		if where == GRB.callback.MIPSOL:
			...
			selected = [(E_dict[E[i]][0],E_dict[E[i]][1]) for i in range(len(E)) if sol[i] == 1 and not(G._node[E[i][0]]['kind']==3 or G._node[E[i][1]]['kind']==3)]
...
def optimization(G1, dummy):
	...
	for i in range(len(E)):
		vars[E_dict[E[i]][0], E_dict[E[i]][1]] = m.addVar(obj=G[E[i][0]][E[i][1]]['cost']+G._node[E[i][1]]['cost'], vtype=GRB.BINARY, name='e_'+str(E_dict[E[i]][0])+'_'+str(E_dict[E[i]][1]))
	m.update()

	# All nodes 
	Q = [q for q in G.nodes() if G._node[q]['kind']==0]
	I = [p for p in G.nodes() if G._node[p]['kind']==1]
	T = [t for t in G.nodes() if G._node[t]['kind']==2]
	E = G.edges()
	...
	Ids = list(set([G._node[p]['cid'] for p in I]))
	...
	for i in range(len(Ids)):
		ids = Ids[i]
		I1 = [p for p in I if G._node[p]['cid']==ids]
	...
	selected = [(E_dict[e][0],E_dict[e][1]) for e in E if solution[E_dict[e][0],E_dict[e][1]] == 1 and not(G._node[e[0]]['kind']==3 or G._node[e[1]]['kind']==3)]
	assert len(subtour(selected)) ==0
	m.write('./Codes/spraying_module_Ratan/mqrppwr/version2/milp/model.sol')
	return Node_dict
