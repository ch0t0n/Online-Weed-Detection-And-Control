...
class Visualize:
    ...
    def __init__(self, polygon, cell_lines, k_path, paths_with_images, socket):
        self.polygon = polygon + [polygon[0]]
        self.updated_path = self.equal_lists(k_path)
        # self.updated_inputs = self.equal_lists(inputs)
        self.cell_lines = cell_lines
        self.paths_with_images = self.equal_lists(paths_with_images)
        self.socket = socket
    ...
    def motion(self, ts):
        ...
        for j in range(len(px[0])):
            ...
            try:
                images = [self.paths_with_images[x][j][1] for x in range(len(self.updated_path))]
                for md in range(1,k+1):
                    if j%md==0:
                        images[md-1] = ""
                images_with_time.append(images)
                print('Current images = ', images, ', time = ', j)
                for image in images:
        ...
        plt.show()
        return time_taken, images_with_time