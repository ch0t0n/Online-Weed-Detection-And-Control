...
if __name__ == "__main__":
    ...
    # print("\n\n Serialized Paths = ", x)
    # Simulation speed
    coverage_runtime, image_paths = visualize.motion(0.1)

    
    k_image_paths = [list(p) for p in zip(*image_paths)]
    
    for path in k_image_paths:
        n = len(path)
        i = 0
        j = 1
    ...
    x = set(zip(*k_image_paths))
    print("\n\n\n Updated zipped", x)

    for i, v in enumerate(x):
        print("Current images = ", v, "time unit = ", i)

    
    with open('Codes/temp_outputs/serialized_paths.txt', "a") as w:
        w.write(str(k_image_paths))
    ...
    with open('Codes/temp_outputs/Experiment_222.csv', "a") as w:
        w.write(output_str)